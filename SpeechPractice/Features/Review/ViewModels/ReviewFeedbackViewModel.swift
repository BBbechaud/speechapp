...
enum ReviewHistoryStore {
    ...
    private static let previewSeedSummaryIDs: Set<UUID> = Set([
    ...
    static func loadRecords() -> [ReviewSessionRecord] {
        if let recordsData = UserDefaults.standard.data(forKey: recordsStorageKey) {
            do {
                ...
                let productionRecords = removingPreviewSeedRecords(from: records)
    ...
    static func record(feedback: ReviewFeedback, in records: [ReviewSessionRecord]) -> [ReviewSessionRecord] {
        ...
        let updatedRecords: [ReviewSessionRecord] = sortedByMostRecent([record] + removingPreviewSeedRecords(from: records))
        save(updatedRecords)
    ...
    private static func loadLegacyRecords() -> [ReviewSessionRecord] {
        ...
        do {
            ...
            // Legacy data is unreadable; clear it rather than manufacturing preview history.
            UserDefaults.standard.removeObject(forKey: legacySummariesStorageKey)
            return []
        }
    }
    ...
    private static func removingPreviewSeedRecords(from records: [ReviewSessionRecord]) -> [ReviewSessionRecord] {
        records.filter { record in
            !previewSeedSummaryIDs.contains(record.summary.id)
        }
    }
